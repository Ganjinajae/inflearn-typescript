// // 라이브러리 로딩
// import  from '라이브러리 이름';
// // 변수, 함수 임포트 문법
// import {} from '파일 상대 경로';
import axios, { AxiosResponse } from 'axios'; // 이름만 있는거는 대부분 라이브러리
// 대부분의 라이브러리, 내부적으로 index.d.ts 파일을 만들어서 제공하고 있음(타입을 정의한 파일) 없으면 @types/**검색 고고
...
function setDeathsList(data: CountrySummaryResponse) {
  ...
  sorted.forEach((value: CountrySummaryInfo) => {
    ...
    //   return;
    // }
    deathsList!.appendChild(li); // '!' null이 아니다. => es-lint가 no-non-null warning => 위에서 'as'로 type assertion을 하면 됨
  });
}
...
function setRecoveredList(data: CountrySummaryResponse) {
  ...
  sorted.forEach((value: CountrySummaryInfo) => {
    ...
    li.appendChild(span);
    li.appendChild(p);
    recoveredList?.appendChild(li); // optional chaining '?' => 우선 순위를 따지자면 type assertion, 물음표, 느낌표
    if (!recoveredList) {
      return;
    }
    // if (!recoveredList) {
